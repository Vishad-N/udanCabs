...
export default function RentalsPage() {
  ...
  const rentals = Array.isArray(data?.data) ? data.data : (data?.data?.data || []);
  ...
  const filteredRentals = rentals.filter((rental: any) =>
    ...
    rental.plateNumber?.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  const handleDelete = async (id: string) => {
    if (confirm('Are you sure you want to delete this rental vehicle?')) {
      try {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-black text-zinc-100 tracking-tight">Two Wheeler Rentals</h1>
          <p className="text-sm text-zinc-400 mt-1">Manage bikes and scooters available for rent</p>
        </div>
        <button
          onClick={handleAdd}
              ...
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center">
                    ...
                    <p className="text-zinc-300 font-medium text-lg">No rental vehicles found</p>
                    <p className="text-zinc-500 mt-1">Try adjusting your search criteria</p>
                  </td>
                </tr>
              ) : (
                filteredRentals.map((rental: any) => (
                  <tr key={rental.id} className="hover:bg-zinc-800/30 transition-colors">
                    <td className="px-6 py-4">
                      <div className="font-bold text-zinc-200">{rental.brand} {rental.model}</div>
                      <div className="text-xs text-zinc-500">{rental.make}</div>
                    </td>
                    <td className="px-6 py-4 font-mono text-xs uppercase">{rental.plateNumber}</td>
                    <td className="px-6 py-4 font-mono text-zinc-200">₹{rental.dailyRate}</td>
                    <td className="px-6 py-4">
                      <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${
                        rental.status === 'AVAILABLE' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' :
                        rental.status === 'RENTED' ? 'bg-amber-500/10 text-amber-400 border border-amber-500/20' :
